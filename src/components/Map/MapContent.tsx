...
export default function MapContent({
  ...
  const { isTracking: isLocationTracking } = useGeolocation({
    map: mapRef.current,
  ...
  return (
    <div 
      className={`absolute inset-0 ${isMobile ? 'pb-12' : ''}`}
      style={{ 
        height: '100%',
        ...
        <div className="leaflet-control-locate leaflet-bar leaflet-control">
          <button
            ...
            className={`
              ...
              ${isLocationTracking ? 'text-blue-500' : 'text-gray-600'}
            `}
            title="Track my location"
          >
            <Navigation size={20} className={isLocationTracking ? 'text-blue-500' : ''} />
          </button>
        </div>

        {isTracking && <LocationTracker onError={setLocationError} />}
